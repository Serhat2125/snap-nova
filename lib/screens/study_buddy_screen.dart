...
//  • 6 robot karakter; kullanıcı birini seçer.
//  • Büyük önizleme: ModelViewer ile gerçek 3D, otomatik döner.
//  • Grid thumbnail'ler: 2D CustomPaint (performans için 6 WebView'dan kaçınıldı).
//  • Renk customization grid + thumbnail için geçerli; 3D model kendi renklerinde.
//  • Seçim + renkler SharedPreferences'ta kalıcı.
//
//  3D model kaynağı: assets/3d/<id>.glb varsa o yüklenir, yoksa CDN URL.
//  Custom .glb için pubspec.yaml'a 'assets/3d/' eklendi ve ilgili dosyaları
//  oraya bırak (bolt.glb, nova.glb, mia.glb, bob.glb, astro.glb, pixel.glb).
// ═══════════════════════════════════════════════════════════════════════════════
...
class _StudyBuddyScreenState extends State<StudyBuddyScreen> {
  ...
  // 3D model URL'leri — KhronosGroup glTF örnek depo + Google modelviewer.dev,
  // ikisi de yıllardır stabil CDN'ler. assets/3d/<id>.glb dosyası varsa o
  // tercih edilir (bkz. _modelSrcFor()).
  ...
  // assets/3d/<id>.glb varsa onu kullan, yoksa CDN URL'ye düş.
  // (Asset varlığı runtime'da kesin doğrulanamaz; pubspec'e ekleyen kullanıcı
  // burada sabit listede de id'sini belirtmeli.)
  static const _localAssetIds = <String>{};
  String _modelSrcFor(_Buddy b) =>
      _localAssetIds.contains(b.id) ? 'assets/3d/${b.id}.glb' : b.modelUrl;
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  // Büyük 3D önizleme — seçili robot otomatik döner.
                  SizedBox(
                    width: 220,
                    height: 220,
                    child: Stack(
                      children: [
                        // Renk halkası (kullanıcının seçtiği primary/accent
                        // 3D modele uygulanamadığı için arkaplan ringinde
                        // canlandırıyoruz).
                        Container(
                          decoration: BoxDecoration(
                        ...
                        // Key: buddy değişince ModelViewer yeniden kurulur ve
                        // yeni .glb yüklenir.
                        ClipOval(
                          child: ModelViewer(
      ...
      return Expanded(
        child: GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: active ? _primary.withValues(alpha: 0.12) : cardBg,
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
                color: active ? _primary : borderC,
                width: active ? 1.6 : 1,
              ),
            ),
...
class _Buddy {
  ...
  const _Buddy(this.id, this.nameKey, this.shape, this.modelUrl);
}
