...
class _MockPaymentContent extends StatelessWidget {
          ...
          child: ListView(
            ...
            children: [
              ...
              _optionTile(
                ...
                title: 'Mobil Ödeme',
                onTap: () => _showMobilePaymentSheet(context),
              ),
              _optionTile(
                context,
              ...
              _optionTile(
                ...
                title: 'Kod Kullan',
                onTap: () => _showRedeemCodeSheet(context),
              ),
  ...
  // ═══════════════════════════════════════════════════════════════════════
  //  Mobil Ödeme Sheet — Vodafone Pay / Turkcell / Türk Telekom
  //  Faturalandırma seçimi. Seçim "geliyor yakında" dialog'u açar; gerçek
  //  ödeme yine Google Play / App Store üzerinden tamamlanır.
  // ═══════════════════════════════════════════════════════════════════════
  void _showMobilePaymentSheet(BuildContext context) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
                  'Abonelik tutarı her ay/yıl faturana eklenir.',
                  style: GoogleFonts.poppins(
                    fontSize: 12,
                    color: const Color(0xFF5F6368),
                  ),
              ...
              _carrierTile(ctx,
                  name: 'Vodafone Pay',
                  desc: 'Vodafone faturana ekle',
                  badgeColor: const Color(0xFFE60000),
                  badgeLetter: 'V'),
              const Divider(height: 1, color: Color(0xFFE5E7EB)),
              _carrierTile(ctx,
                  name: 'Turkcell Mobil Ödeme',
                  desc: 'Turkcell faturana ekle',
                  badgeColor: const Color(0xFFFFB800),
                  badgeLetter: 'T'),
              const Divider(height: 1, color: Color(0xFFE5E7EB)),
              _carrierTile(ctx,
                  name: 'Türk Telekom Mobil Ödeme',
                  desc: 'Türk Telekom faturana ekle',
                  badgeColor: const Color(0xFF003DA5),
                  badgeLetter: 'T'),
            ],
          ),
        );
  ...
  Widget _carrierTile(BuildContext context, {
    required String name,
    required String desc,
    required Color badgeColor,
    required String badgeLetter,
  }) {
    return InkWell(
      onTap: () {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  //  Kod Kullan Sheet — promosyon/hediye kodu girişi.
  //  Format kontrolü minimal; gerçek doğrulama backend'de yapılır (Google
  //  Play sheet'inde alınan kod arka planda işlenir).
  // ═══════════════════════════════════════════════════════════════════════
  void _showRedeemCodeSheet(BuildContext context) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) {
        return StatefulBuilder(builder: (ctx2, setS) {
          final canRedeem = codeCtrl.text.trim().length >= 4;
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                              'Promosyon, hediye veya davet kodunu yaz; ödemeye uygulanır.',
                              style: GoogleFonts.poppins(
                                fontSize: 11.5,
                                color: const Color(0xFF5F6368),
                                height: 1.35,
                    ...
                    child: FilledButton(
                      ...
                      onPressed: canRedeem
                          ? () async {
                              ...
                              if (!RegExp(r'^[A-Z0-9\-]{4,20}$')
                                  .hasMatch(code)) {
                                setS(() => errorMsg =
                                    'Kod sadece harf, rakam ve - içerebilir');
                                return;
                              }
                              // Loading state — buton kilitlensin
                              setS(() => errorMsg = null);
                              ...
                              switch (result) {
                                case RedeemResult.success:
                                  msg = 'Kod uygulandı — 7 gün Premium aktif!';
                                  bg = const Color(0xFF22C55E);
                                  break;
                                case RedeemResult.alreadyUsed:
                                  msg = 'Bu hesap zaten bir davet kodu kullandı.';
                                  bg = const Color(0xFFF59E0B);
                                  break;
                                case RedeemResult.selfReferral:
                                  msg = 'Kendi kodunu kullanamazsın.';
                                  bg = const Color(0xFFEF4444);
                                  break;
                                case RedeemResult.invalidCode:
                                  msg =
                                      'Kod formatı hatalı veya tanınmadı (QuAls-XXXXXX).';
                                  bg = const Color(0xFFEF4444);
                                  break;
                                case RedeemResult.ownerNotFound:
                                  msg = 'Kodun sahibi bulunamadı.';
                                  bg = const Color(0xFFEF4444);
                                  break;
                                case RedeemResult.deviceAlreadyUsed:
                                  msg =
                                      'Bu cihazdan daha önce bir kod kullanıldı.';
                                  bg = const Color(0xFFEF4444);
                                  break;
