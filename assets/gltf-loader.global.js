// GLTFLoader r160 + toTrianglesDrawMode — klasik script (window.THREE'den beslenir)
(function(){
...
// ===== ORTAK YARDIMCILAR: base64 gomulu GLB modelleri =====
window._glbCache = window._glbCache || {};
window.glbFromB64 = function(b64, cb) {
  if (!window.GLTFLoader || !b64) { cb(null); return; }
  try {
    const bin = atob(b64); const u8 = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) u8[i] = bin.charCodeAt(i);
    new window.GLTFLoader().parse(u8.buffer, '', function(g){ cb(g.scene); }, function(){ cb(null); });
  } catch (e) { cb(null); }
};
window.fitGlb = function(obj, target) {
...
window.ensureGlb = function(key, b64, cb) {
  if (window._glbCache[key]) { cb(window._glbCache[key]); return; }
  window.glbFromB64(b64, function(m){ if (m) window._glbCache[key] = m; cb(m); });
};
window.glbSelfLight = function(g, k) {
  g.traverse(function(o){ if (o.isMesh && o.material && o.material.color) {
    o.material.emissive = o.material.color.clone().multiplyScalar(k === undefined ? 0.3 : k);
