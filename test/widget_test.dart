...
class _MainScreenState extends State<MainScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildSecondPage() {
    ...
    return Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 450), child: Padding(padding: const EdgeInsets.all(20.0), child: Column(children: [Align(alignment: Alignment.centerLeft, child: IconButton(icon: const Icon(Icons.arrow_back, color: Colors.cyanAccent), onPressed: _goBack)), Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.cyanAccent.withValues(alpha: 0.3))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: const [Icon(Icons.auto_awesome, color: Colors.cyanAccent, size: 24), SizedBox(width: 8), Text("SnapNova", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.cyanAccent))]), const SizedBox(height: 12), const Text("Her derste her konuda istediğin her şeyi sor SnapNova anında çözsün.", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.white, height: 1.3))])), const SizedBox(height: 16), Expanded(child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.05), borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.cyanAccent.withValues(alpha: 0.3))), child: GridView.builder(itemCount: subjects.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 2.8, crossAxisSpacing: 10, mainAxisSpacing: 2), itemBuilder: (context, index) { final subject = subjects[index]; return Column(children: [Expanded(child: Row(children: [Icon(subject['icon'] as IconData, color: subject['color'] as Color, size: 18), const SizedBox(width: 10), Expanded(child: Text(subject['name'] as String, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.white.withValues(alpha: 0.9)), overflow: TextOverflow.ellipsis))])), Divider(color: Colors.white.withValues(alpha: 0.1), thickness: 0.5)]); }))), const SizedBox(height: 12), GestureDetector(onTap: () => _pageController.nextPage(duration: const Duration(milliseconds: 500), curve: Curves.easeInOut), child: Container(width: double.infinity, height: 52, decoration: BoxDecoration(gradient: const LinearGradient(colors: [Colors.cyanAccent, Colors.blueAccent]), borderRadius: BorderRadius.circular(20)), child: const Center(child: Text("Devam Et", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87))))), const SizedBox(height: 16), Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(3, (index) => AnimatedContainer(duration: const Duration(milliseconds: 300), margin: const EdgeInsets.symmetric(horizontal: 5), height: 4, width: _currentPage == index ? 32 : 14, decoration: BoxDecoration(color: _currentPage == index ? Colors.cyanAccent : Colors.grey.withValues(alpha: 0.4), borderRadius: BorderRadius.circular(10)))))]))));
  }
...
class CameraScreen extends StatefulWidget {
  ...
  const CameraScreen({super.key, required this.isSingleMode, required this.isFlashOn, required this.onModeChange, required this.onFlashToggle, required this.onPhotoTaken, required this.onGalleryTap, required this.onCalculatorTap, required this.onBackPressed});
  @override
...
class _CameraScreenState extends State<CameraScreen> {
  ...
  Widget _coloredIconButton(IconData icon, String label, Color color, VoidCallback onTap) { return GestureDetector(onTap: onTap, child: Column(mainAxisSize: MainAxisSize.min, children: [Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: color.withValues(alpha: 0.15), shape: BoxShape.circle, border: Border.all(color: color.withValues(alpha: 0.4))), child: Icon(icon, color: color, size: 24)), const SizedBox(height: 4), Text(label, style: TextStyle(fontSize: 9, color: color.withValues(alpha: 0.9)), textAlign: TextAlign.center)])); }
}
...
class _CalculatorScreenState extends State<CalculatorScreen> {
  ...
  Color _getButtonColor(String label) { if (label == "C" || label == "⌫") { return Colors.redAccent.withValues(alpha: 0.7); } else if (label == "=") { return Colors.cyanAccent.withValues(alpha: 0.7); } else if (["+", "-", "×", "÷", "%"].contains(label)) { return Colors.orangeAccent.withValues(alpha: 0.7); } else if (["sin", "cos", "tan", "log", "ln", "√", "x²", "x^y", "π", "e"].contains(label)) { return Colors.purpleAccent.withValues(alpha: 0.7); } else { return Colors.white.withValues(alpha: 0.1); } }
}
