...
| `https://qualsar.app/privacy` | Privacy Policy URL (canlı) |
| `https://qualsar.app/terms` | Kullanım Koşulları URL (canlı) |
...
- **Privacy Policy:** `https://qualsar.app/privacy`
...
## ✅ ADIM 10.5 — ⚠️ KRİTİK: Ödeme doğrulama backend kurulumu

SKU'ları oluşturduktan sonra **bu adımı yapmazsan kullanıcı para öder ama premium aktive olmaz.**
Süre: 20-30 dakika. Bir kez yap, unut.

### A) Google Play service account (Android receipt doğrulama)

1. **Google Cloud Console** → https://console.cloud.google.com/iam-admin/serviceaccounts?project=qualsar2-640f0
2. **"+ HİZMET HESABI OLUŞTUR"**
   - Ad: `play-billing-verifier`
   - Açıklama: `Google Play Developer API erişimi`
3. **Roller atama adımını ATLA** (Play Console'da ayrı izin vereceğiz)
4. Oluşturulan hesabın 3-nokta menüsü → **Anahtarları yönet** → **Anahtar ekle** → **JSON** → indir
5. İndirilen JSON'u **gizli tut** — `~/Downloads/play-billing-verifier.json` gibi bir yere koy
6. Play Console → **Ayarlar** → **API erişimi** → Google Cloud projesini bağla (qualsar2-640f0)
7. Aynı sayfada yeni service account'u listede gör → **Erişim ver** → izinler:
   - ☑ Finansal verileri görüntüle
   - ☑ Siparişleri ve abonelikleri yönet
8. **Kullanıcıyı davet et** → onaylayınca aktif olur

### B) App Store Connect shared secret (iOS receipt doğrulama)

> iOS yayını şu an yapmıyorsan bu adımı atla; sonra döneriz.

1. **App Store Connect** → **Apps** → **QuAlsar** → **App Information**
2. Sağ alttaki **"App-Specific Shared Secret"** → **Generate** → kopyala (32 karakterlik hex)

### C) Secret'ları Firebase Functions'a yükle

PowerShell'de proje kök dizinde:

```powershell
# Android service account JSON'u secret'a aktar
Get-Content "$HOME\Downloads\play-billing-verifier.json" -Raw | firebase functions:secrets:set GOOGLE_PLAY_SA --project qualsar2-640f0

# iOS shared secret (sadece iOS yayınlıyorsan)
"APP_STORE_PASTE_HERE" | firebase functions:secrets:set APP_STORE_SHARED_SECRET --project qualsar2-640f0
```

Komut "Successfully created" yazınca tamam.

### D) Real-Time Developer Notifications (Google Play webhook)

Kullanıcı abonelik iptal ederse veya yenilenirse, Google bize webhook atar. O zaman premium otomatik kapanır/yenilenir.

1. **Google Cloud Console** → **Pub/Sub** → **Topics** → https://console.cloud.google.com/cloudpubsub/topic/list?project=qualsar2-640f0
2. **"+ KONU OLUŞTUR"** → ID: `qualsar-rtdn` → Oluştur
3. Konu detayına git → **İZİNLER** → **+ HESAP EKLE**
   - Yeni asıl: `google-play-developer-notifications@system.gserviceaccount.com`
   - Rol: **Pub/Sub Publisher**
   - Kaydet
4. **Play Console** → Uygulamayı seç → **Para kazanma** → **Para kazanma kurulumu**
5. Aşağı kaydır → **Gerçek zamanlı geliştirici bildirimleri**
   - Konu adı: `projects/qualsar2-640f0/topics/qualsar-rtdn`
   - **Test bildirimi gönder** tuşuna bas → "OK" yazısı = handshake başarılı

### E) Cloud Functions'ı deploy et

PowerShell'de:

```powershell
firebase deploy --only functions:verifyPurchase,functions:rtdnWebhook --project qualsar2-640f0
```

Deploy bitince:
- Telefonda Internal Testing build'i ile test satın alma yap (Play Console → License testing'ten lisans testçisi olarak kendini ekle, ücretsiz satın alma)
- Beklenen: "Premium aktif" badge'i 5-10 saniyede ekrana düşer
- Firebase Console → Functions → Loglar'da `[verifyPurchase] OK uid=... expires=...` satırını gör

### Test sırasında sorun çıkarsa

| Log mesajı | Çözüm |
|---|---|
| `failed-precondition: GOOGLE_PLAY_SA secret konfigüre edilmemiş` | C-adımı yapılmamış / deploy edilmemiş |
| `Google Play response'da expiryTime yok` | Service account'a Play Console izni A-7'de verilmemiş |
| `App Store HTTP 21002` | iOS receipt bozuk — sandbox'ta test ediyorsan TestFlight'tan al |
| `purchaseToken için uid mapping bulunamadı` | İlk satın alma sırasında mapping yazılamamış (firestore.rules eskimiş olabilir) — `firebase deploy --only firestore:rules` |
...
[ ] 10.5 ⚠️ ÖDEME BACKEND KURULUMU (Adım 10.5 detaylı — service account + secret + RTDN topic + deploy)
[ ] 11. Ülkeleri seç
